...
def utdc_at_threshold(x,y,thr):
    ...
    return (1-2*(i/n) + empirical_copula(x,y,thr,thr) ) / (1-(i/n)) 
...
def utdc_cfg(x,y):
    ...
    return 2-2*np.exp( 1/n * np.sum(np.log(np.sqrt(np.log(1/scaled_ranks_x)*np.log(1/scaled_ranks_y))/np.log(1/(max_ecdf**2)) ) ))
